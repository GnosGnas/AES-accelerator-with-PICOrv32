...
void send_stat(bool status) {
	if (status) *((volatile int *)STAT_ADDR) = 1;
	else *((volatile int *)STAT_ADDR) = 0;
}
...
inline void ChecknWait (int addr) {   //Timeout can be added here - Wasn't added to save few cycles
